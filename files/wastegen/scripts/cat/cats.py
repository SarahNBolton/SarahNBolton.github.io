...
from scripts.cat.mutations import Mutations
...
class Cat:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.mutations = mutations if mutations else Mutations()
        ...
        """if not loading_cat:
            self.mutate = Mutations()
            self.mutations = self.mutate.get_mutation_dict(self.mutate.init_mutations([Cat.fetch_cat(i) for i in (self.parent1, self.parent2) if i]))
            print(self.mutations)"""
        """else:
            self.mutate = Mutations()
            cat = Cat.fetch_cat(ID)
            print(cat)
            self.genotype = {}
            self.genotype["ear_size"] = cat[str("ear_size")]
            self.genotype["ear_type"] = cat["ear_type"]
            self.genotype["eye_type"] = cat["eye_type"]
            self.genotype["eye_count"] = cat["eye_count"]
            self.genotype["tail_type"] = cat["tail_type"]
            self.genotype["tail_count"] = cat["tail_count"]
            self.genotype["teeth"] = cat["teeth"]
            self.genotype["horns_presence"] = cat["horns_presence"]
            self.genotype["horns"] = cat["horns"]
            self.genotype["wings_presence"] = cat["wings_presence"]
            self.genotype["wings_type"] = cat["wings_type"]
            self.genotype["wings"] = cat["wings"]
            self.genotype["extras_presence"] = cat["extras_presence"]
            self.genotype["extras"] = cat["extras"]
            self.mutations = self.mutate.get_mutation_dict(self.genotype)"""
    ...
    def init_generate_cat(self, skill_dict):
        ...
        self.mutate = Mutations()
        #self.mutations = self.mutate.get_mutation_dict(self.mutate.init_mutations([Cat.fetch_cat(i) for i in (self.parent1, self.parent2) if i]))
        self.mutations = self.mutate.get_mutation_dict(self.mutate.generate_genotype())
        print(self.mutations)

        self.ear_size_genes = self.mutations[0]
        self.ear_type_genes = self.mutations[1]
        self.eye_type_genes = self.mutations[2]
        self.eye_count_genes = self.mutations[3]
        self.tail_type_genes = self.mutations[4]
        self.tail_count_genes = self.mutations[5]
        self.teeth_genes = self.mutations[6]
        self.horns_presence_genes = self.mutations[7]
        self.horns_genes = self.mutations[8]
        self.wings_presence_genes = self.mutations[9]
        self.wings_type_genes = self.mutations[10]
        self.wings_genes = self.mutations[11]
        self.extras_presence_genes = self.mutations[12]
        self.extras_genes = self.mutations[13]
    ...
    def get_save_dict(self, faded=False):
        if faded:
            ...
            print(self.wings_presence_genes)
            return {
                "ID": self.ID, 
                "name_prefix": self.name.prefix,
                "name_suffix": self.name.suffix,
                ...
                "ear_size": self.ear_size_genes,
                "ear_type": self.ear_type_genes,
                "eye_type": self.eye_type_genes,
                "eye_count": self.eye_count_genes,
                "tail_type": self.tail_type_genes,
                "tail_count": self.tail_count_genes,
                "teeth": self.teeth_genes,
                "horns_presence": self.horns_presence_genes,
                "horns": self.horns_genes,
                "wings_presence": self.wings_presence_genes,
                "wings_type": self.wings_type_genes,
                "wings": self.wings_genes,
                "extras_presence": self.extras_presence_genes,
                "extras": self.extras_genes,
            }
